...
from .fpn import FPN_hrnet
import torch.nn as nn
from .loss import GenTargets, LOSS, coords_fmap2orig
import torch
from .config import DefaultConfig
from .backbone.hrnet import hrnetv2
...
class FCOS(nn.Module):
    def __init__(self, config=None):
        ...
        self.backbone = hrnetv2()
        self.fpn = FPN_hrnet(features=64)
        self.head = ClsCntRegHead(
            64, config.class_num, config.use_GN_head, config.cnt_on_reg, config.prior
        )
        self.config = config
...
class DetectHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        topk_ind = torch.topk(cls_scores, max_num, dim=-1, largest=True, sorted=True)[
            1
        ]  # [batch_size,max_num]
        _cls_scores = []
        _cls_classes = []
    ...
    def _post_process(self, preds_topk):
        ...
        scores, classes, boxes = (
            torch.stack(_cls_scores_post, dim=0),
            torch.stack(_cls_classes_post, dim=0),
